    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.status == 404) {
                ...
                //console.log(response.fieldGolonganDarah.nama_bidang_pendidikan)
                $('#detail_golongan_darah').text(response.fieldGolonganDarah
                    .golongan_darah);
                $('#detail_keterangan').text(response.fieldGolonganDarah.keterangan);
            }
        }
    });
...
$(document).on('click', '.deleteBtn', function(e) {
    ...
    $('#deleting_id').val(fgd_id);
});

//-------------------------------------------------------------------------------------------------
//Ajax Delete Data
//-------------------------------------------------------------------------------------------------
$(document).on('click', '.delete_golonganDarah', function(e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status == 400) {
                ...
                $('.delete_golonganDarah').text('Hapus');
            } else {
                //$('#save_msgList').html("");
...
<div class="content">
    <div class="card">
        ...
        <table id="golonganDarahTable" class="table datatable-basic table-striped">
            <thead>
                <tr>
            ...
            <tbody>
                @if (isset($fieldGolonganDarah) && count($fieldGolonganDarah) > 0)
                @foreach ($fieldGolonganDarah as $fGD)
                <tr>
                    <td>{{ $fGD -> golongan_darah }}</td>
                    <td>{{ $fGD -> keterangan }}</td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="container">
                        <div class="row mb-2">
                            <label for="detail_golongan_darah" class="col-lg-4 col-form-label">
                                Golongan Darah:</label>
                            <div class="col-lg-7">
                                <label id="detail_golongan_darah" class="col-form-label"></label>
                            </div>
                        </div>
                        <div class="row mb-2">
    ...
    <div id="deleteModal" class="modal fade" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Hapus Data Golongan Darah</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form id="deleteGolonganDarahForm">
                    @csrf
                    <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary delete_golonganDarah">Hapus</button>
                    </div>
                </form>
            </div>
        </div>
