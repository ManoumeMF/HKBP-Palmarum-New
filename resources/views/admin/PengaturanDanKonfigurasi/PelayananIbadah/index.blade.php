...
$(document).on('click', '.detailBtn', function(e) {
    ...
    $.ajax({
        method: "GET",
        url: "{{ route('PelayananIbadah.detail') }}",
        data: {
            id: fe_id
        },
        success: function(response) {
            if (response.status == 404) {
                ...
                $('#detail_nama_pelayan').text(response.pelayananibadah.nama_pelayanan_ibadah);
                $('#detail_keterangan').text(response.pelayananibadah.keterangan);
...
$(document).on('click', '.delete_ressort', function(e) {
    ...
    $.ajax({
        type: "DELETE",
        url: "{{ route('PelayananIbadah.delete') }}",
        data: data,
        dataType: "json",
        success: function(response) {
...
<div class="content">
    <div class="card">
        <div class="card-header d-flex">
            <h5 class="mb-0">Daftar Pelayanan Ibadah</h5>
            <div class="ms-auto">
                <a class="btn btn-primary" href="{{ route('PelayananIbadah.create') }}"><i
        ...
        <table id="bidangPendidikanTable" class="table datatable-basic table-striped">
            <thead>
                <tr>
                    <th>Nama Pelayanan Ibadah</th>
                    <th>Keterangan</th>
                    <th class="text-center">Tindakan</th>
                </tr>
            </thead>
            <tbody>
                @if (isset($pelayananibadah) && count($pelayananibadah) > 0)
                @foreach ($pelayananibadah as $r)
                <tr>
                            <td>{{ $r->nama_pelayanan_ibadah }}</td>
                            <td>{{ $r->keterangan }}</td>
                        ...
                        <div class="d-inline-flex">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-end">
                                    <button type="button" value="{{ $r -> id_pelayanan_ibadah }}"
                                        class="dropdown-item text-info detailBtn">
                                        <i class="ph-list me-2"></i>Detail
                                    </button>
                                    <button type="button" value="{{ $r -> id_pelayanan_ibadah }}"
                                        class="dropdown-item text-secondary">
                                        <a href="{{ route('PelayananIbadah.edit', $r -> id_pelayanan_ibadah) }}"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Detail Distrik</h5>
                ...
                <div class="container">
                    <div class="row mb-2">
                        <label for="detail_nama_bank" class="col-lg-4 col-form-label">
                            Nama Pelayanan Ibadah:</label>
                        <div class="col-lg-7">
                            <label id="detail_nama_pelayan" class="col-form-label"></label>
                        </div>
                    </div>
                    <div class="row mb-2">
...
<div id="deleteModal" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Hapus Data Pelayanan Ibadah</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
